...
Original file is located at
    https://colab.research.google.com/drive/1IigOtCbMEEACsGLR905neRwfwkn5fPEl
...
class SignLanguage:
    ...
    def prepare_data(self, images, labels):
        ...
        train_size = int(images.shape[0]*0.66)

        #Uncomment for CNN
        train_data = images[:train_size,:]
        train_data = np.array([np.reshape(x, (28,28,1)) for x in train_data])
        train_labels = labels[:train_size]
        train_labels_categorical = to_categorical(train_labels,num_classes=labels.max()+1)

        test_data = images[train_size:,:]
        test_data = np.array([np.reshape(x,(28,28,1)) for x in test_data])
        test_labels = labels[train_size:]
        test_labels_categorical = to_categorical(test_labels,num_classes=labels.max()+1)

        self.data = {
            "train": (train_data,train_labels_categorical), # (x_train, y_train)
            "test" : (test_data,test_labels_categorical), # (x_test, y_test)
        }

    def train(self, batch_size:int=128, epochs:int=50, verbose:int=1):
        """
        Use model.fit() to train your model.
        Make sure to return the history for a neat visualization.
    ...
    def predict(self, data):
        ...
        to_test_data = np.array([np.reshape(x, (28,28,1)) for x in data])
        results = np.argmax(self.model.predict(to_test_data) , axis = 1)
